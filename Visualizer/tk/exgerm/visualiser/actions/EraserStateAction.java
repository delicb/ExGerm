...
public class EraserStateAction extends ExGAction {
	
	private ViewManager viewManager;
	
	public EraserStateAction(ViewManager _vm){
		putValue(NAME, "Eraser State"); //$NON-NLS-1$
		putValue(SMALL_ICON, loadIcon("images/erase.png"));
		putValue(SHORT_DESCRIPTION, "Erase node or edge"); //$NON-NLS-1$
		viewManager = _vm;
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		try{
			viewManager.getActiveView().getStateManager().setNextState(StateManager.States.ERASER, null);
		}catch(Exception e){
